...
* Suggests high false positive rate. Do the opposite of recommendations for "Too few spots": i.e. increase strictness of filtering criteria. If possible, users may attempt manual decoding on a small region of one or two FOVs, and ensure that their pipeline returns similar results.
...
* **Images look like noise when loaded.** We expect this to be more likely for `.dax` images and may be due to incorrect image size input, or incorrect endian-ness (default: `little`). Alternatively, user has attempted to load a 3D image stack as 2D image -- which will cause problems regardless of image file format.
  ...
  + **Potential solution**: If reason is because of loading a 3D image stack -- which default `masmr` is currently under-equipped to handle -- users will need to modify image reading on their own. One quick fix is to perform a maximum intensity projection (MIP), to collapse a 3D image stack per channel into a single 2D image per channel. [An alternative is to treat each slice as a separate image (but this can get complicated).] 
  
Some example code for performing a MIP -- assuming images are `ome.tif` files that can be read by `RBioFormats` -- is provided below. The idea is to load an image Z-slice by Z-slice, compare intensities between Z-slices, and selecting the brightest pixels. This idea carries over to other image file formats, with some variation in execution: e.g. the convenient `subset = list( z=zSlice )` function is unlikely to work for non-`ome.tif` file formats.
  
```{r troubleshooting-3D-MIP-ome, eval=FALSE}
nZslices = ... # The number of z slices
...
for( fovName in params$fov_names ){
  ...
  ## Image reading: to replace default readImageList()
  for( zSlice in 1:nZslices){
    
    ## Read image list one z slice at a time
    imList <- readImageList(
      
      ## Should automatically detect params$current_fov, but will make explicit here:
      chosenFOV = params$current_fov,
      ## Turn off safe loading 
      safeLoad = F, 
      ## Add an additional subset parameter to load image by z-slice
      subset = list( z=zSlice )
      
      )
    
    ## Now split image list by channel to obtain nBit images
    channel_imList <- list()
    current_image_idx = 1
    current_channel_idx = 1
    for( i in 1:ncol(params$ordered_codebook) ){
      bit_name <- colnames(params$ordered_codebook)[i]
      current_im <- imList[[ current_image_idx ]]
      channel_imList[[ bit_name ]] <- current_im[,,current_channel_idx]
      current_channel_idx = current_channel_idx + 1
      if(current_channel_idx > length(params$resolutions$channel_order)){
        current_channel_idx <- 1
        current_image_idx <- current_image_idx + 1
      }
    }
    imList <- channel_imList
    
    if( zSlice==1 ){
      maximumIntensityProjection <- imList
    }else{
      maximumIntensityProjection <- setNames( lapply(1:length(maximumIntensityProjection), function(i){
        
        ## Grab your data
        current_bit_name <- names(maximumIntensityProjection)[i]
        current_mip_image <- maximumIntensityProjection[[i]]
        image_to_compare <- imList[[current_bit_name]]
        
        ## Figure out which image is brighter, and replace with the higher intensity
        difference_between_images <- current_mip_image - image_to_compare
        current_mip_image[difference_between_images < 0] <- image_to_compare[difference_between_images < 0]
        
        return( current_mip_image )
        
      }), names(maximumIntensityProjection) )
    }
  }
  
  ## If rest of the pipeline uses `imlist` object:
  imList <- maximumIntensityProjection
  
  ## Proceed with rest of pipeline
  ...
}
```

Below is a theoretical example of MIP -- assuming images are `.dax` files (i.e. _cannot_ be read by `RBioFormats`), and assuming that each `.dax` file is unique to an image and a channel (and so no channel splitting -- see above -- is necessary), while containing the complete stack of Z-slices.

```{r troubleshooting-3D-MIP-dax, eval=FALSE}
nZslices = ... # The number of z slices
nImageRows = ... # The number of image rows: width
nImageCols = ... # The number of image columns: height
...
for( fovName in params$fov_names ){
  ...
  ## Read all z-slices simultaneously
  ## WARNING: may run into memory management problems
  imList <- readImageList(
    
    ## Should automatically detect params$current_fov, but will make explicit here:
    chosenFOV = params$current_fov,
    ## Turn off safe loading (does not matter for .dax, but will stay consistent) 
    safeLoad = F, 
    ## The trick here is to load every image as a long column, with each zslice as a row
    ## Better to input nZslices, nImageRows, and nImageCols yourself -- reading image meta data might run into issues
    nrows = nZslices,
    ncols = nImageRows * nImageCols
    
    )
  
  ## imList should now be a list of matrices, each matrix containing ALL z-stacks as rows
  imList <- setNames(lapply(1:length(imList), function(i){
    
    current_z_stack <- imList[[i]]
    maximum_intensity_projection <- apply(current_z_stack, 1, max)
    maximum_intensity_projection <- matrix(maximum_intensity_projection, nrow=nImageRows, ncol=nImageCols)
    return(maximum_intensity_projection)
    
  }), names(imList))
  
  ## Presumably, getAnchorParams also has not worked, so users will have to manually decide
  minBrightness = ...
  maxBrightness = ...
  params$brightness_max <- rep(maxBrightness, ncol(params$ordered_codebook))
  params$brightness_min <- rep(minBrightness, ncol(params$ordered_codebook))
...
If users insist on treating each z-slice separately, users may loop through the entire pipeline n z-slice times.

```{r troubleshooting-3D-loopZ, eval=FALSE}
nZslices = ... # The number of z slices
nImageRows = ... # The number of image rows: width
nImageCols = ... # The number of image columns: height

for( zSlice in nZslices ){
  ...
  ## Change the output directory: so that each z-slice is given its own folder
  params$out_dir <- paste0(params$parent_out_dir, '/IM/Z', zSlice)
  if(!dir.exists(params$out_dir)){
    dir.create(params$out_dir)
  }
  
  ## Read current z slice
  if( params$im_format == '.dax' ){
    
    ## Read .dax as written above
    imList <- readImageList(
      chosenFOV = params$current_fov,
      safeLoad = F, 
      nrows = nZslices,
    ...
    ## Get the current zslice only
    imList <- setNames(lapply(1:length(imList), function(i){
      current_z_stack <- imList[[i]]
      current_z_slice <- current_z_stack[zSlice,]
      current_z_slice <- matrix(current_z_slice, nrow=nImageRows, ncol=nImageCols)
      return(current_z_slice)
      }), names(imList))
